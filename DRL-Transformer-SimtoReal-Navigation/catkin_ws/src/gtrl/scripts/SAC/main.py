...
file_name = "SAC result_VLP_16 500_2nd"
...
def evaluate(network, eval_episodes=10, epoch=0):
    ...
    while ep < eval_episodes:
        ...
        avg_reward = 0.0
        #________________________i added
        identified = False
        ...
        while not done and count < max_steps:
            ...
            obs_list.append(obs_)
            count += 1
            #_____________________i added
            if target and not identified:
        ...
        if done and count < max_steps and not env.collision:
            total_success += 1
    reward = statistics.mean(avg_reward_list)
    col = env.collision
    #_________________________i added
    avg_reward = statistics.mean(avg_reward_list)
    collision_count = env.collision
    collision_rate = env.collision / eval_episodes
    success_rate = total_success / eval_episodes
    poi = total_identifications / eval_episodes
    ...
    print("..............................................")
    #________________i added
    writer.add_scalar('Metrics/AverageReward', avg_reward, epoch)
    writer.add_scalar('Metrics/CollisionCount', collision_count, epoch)
    writer.add_scalar('Metrics/SuccessRate', success_rate, epoch)
    writer.add_scalar('Metrics/ProbabilityOfIdentification', poi, epoch)
    writer.add_scalar('Metrics/CollisionRate', collision_rate, epoch)
    #__________________________
    return reward,avg_reward, collision_rate, success_rate, poi
...
if __name__ == "__main__":
    ...
    if pre_buffer:
        data_dir = '/home/seher/Project/src/DRL-Transformer-SimtoReal-Navigation/catkin_ws/src/gtrl/scripts'
    ...
    #_______________________i added
    total_success_training = 0
    total_identifications_training = 0
        #_________________________________________
    for ep in tqdm(range(0, max_episodes), ascii=True):
        #______________i added 
        ...
        s, goal = env.reset()
        min_reward = -100  # Replace with the known minimum reward
        max_reward = 100   # Replace with the known maximum reward
        for i in range(4):
            s_list.append(s)
        ...
        for timestep in range(max_steps):
            ...
            if timestep == 0:
                ...
                for i in range(4):
                    s_list.append(s_)  
                #______________________i added      first   
                episode_reward += reward

                # Check for success
                if np.array_equal(goal, target) and not env.collision:
                    episode_success = True

                # Check for identification only if not already identified
                if not episode_identified and np.array_equal(goal, target):
                    episode_identified = True
                    #___________________________i added
                if done:
                    print("Bad Initialization, skip this episode.")
                    break
            ...
            if done or timestep == max_steps-1:
                ...
                #_________________________________i added first
                if episode_success:
                    total_success_training += 1

                success_rate_training = total_success_training / max(1, ep_real)
                collision_rate_training = env.collision / max(1, ep_real)
                poi = total_identifications_training / (ep_real + 1)
                #_____________________________________first
                ...
                collision_rate_training = env.collision / max(1, ep_real)
                #_________________________________i added third
                overak_r = reward_mean_list[-1]  # Get the Overak R

                # Normalize Overak R to [0.0, 1.0] for success rate
                success_rate_normalized = max(0.0, min(1.0, (overak_r - min_reward) / (max_reward - min_reward)))
...
#___________________________________________
                writer.add_scalar('Metrics/Collision_Reward', np.mean(reward_collision_list), ep)
                writer.add_scalar('Metrics/AverageReward', reward_mean_list[-1], ep)
                writer.add_scalar('Metrics/Collisions', env.collision, ep)
#________________________________________________________________________________
                writer.add_scalar('Metrics/CollisionCount', env.collision, ep_real)
                writer.add_scalar('Metrics/TrainingCollisionRate', collision_rate_training, ep_real)
                writer.add_scalar('Metrics/TrainingSuccessRate', success_rate_training, ep_real)
                writer.add_scalar('Metrics/ProbabilityOfIdentification', poi, ep_real)
                writer.add_scalar('Metrics/NormalizedSuccessRate', success_rate_normalized, ep_real)
